...
export function NeuronParticles() {
  const { state, memories } = useAIStore();
  ...
  const positions = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      const radius = 2 + Math.random() * 3; // slightly bigger sphere to fit memory nodes inside
  ...
  useFrame(({ clock }) => {
    ...
    materialRef.current.opacity = Math.min(1, 0.35 + Math.abs(Math.sin(time * profile.pulseSpeed)) * (profile.pulseAmp * 4));
  ...
  return (
    <group>
      ...
      {/* MEMORY NODES INSIDE PARTICLE CLOUD */}
      {memories.map((m) => (
